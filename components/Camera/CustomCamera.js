...
import {
  Alert, StyleSheet, Text, View, TouchableOpacity, Slider, Platform,
} from 'react-native';
...
import {
  Constants, Camera, FileSystem, Permissions, BarCodeScanner, Notifications,
} from 'expo';
...
class CustomCamera extends React.Component {
  ...
  showPopup = (response) => {
    ...
    response.forEach((recognizedObject) => {
      ...
      message
        += `${modelType[recognizedObject.type]} was last seen at ${recognizedObject.lastSeen}`
        + '\n';
    });
    this.popup.show({
